...
if __name__ == '__main__':
    ...
    parser.add_argument('-L', nargs=4, help="Co-ordinates to restrict query to. Write in format Xmin Ymin Xmax Ymax")
    ...
    if input("Do you want to create a database (Y/N)?").lower() == "y":
        if XML_NAME is None:
            ...
            min_x = custom_min((COORDS[0],))
            min_y = custom_min((COORDS[1],))
            max_x = custom_max((COORDS[2],))
            max_y = custom_max((COORDS[3],))
            request_string = "(way[\"highway\"]({},{},{},{});node(w););out body;"
            req.urlretrieve('https://overpass-api.de/api/interpreter?data=' +
                                parse.quote_plus(request_string.format(min_x, min_y, max_x,max_y),
                                                safe="[]();.,"),
                             os.path.join(os.getcwd(), "osm_d.osm"))
            XML_NAME = 'osm_d.osm'
        print(PREFIX_STRING)
        if os.name.lower() in ['windows', 'nt']:
            if subprocess.call("powershell -Command \"$env:PGPASSWORD='{}'; psql -U {} -c 'create database {};'\"".format(DB_PASSWORD, DB_USER, DB_NAME)) != 0:
                sys.exit(1)
            ...
            if subprocess.run(['psql', PREFIX_STRING, 'create database', '-e', 'utf8', '{}'.format(DB_NAME)], shell=True, capture_output=True).stderr is not None:
                sys.exit(1)
            if subprocess.run(['psql', PREFIX_STRING, 'create extension postgis']).stderr is not None:
                sys.exit(1)
            if subprocess.run(['psql', PREFIX_STRING, 'create extension hstore']).stderr is not None:
                sys.exit(1)
        if os.name.lower() in ['windows', 'nt']:
            try:
                ...
                output = subprocess.run(
                    [path + '\\osm2pgsql',
                     '-S', '{}\\default.style'.format(path), '-W', '-U', DB_USER, '-d', DB_NAME, '-r', 'xml', '-H', DB_HOST, '-P', DB_PORT, XML_NAME],
            ...
            output = subprocess.run(
                ['osm2pgsql',
                 '-s', '-W', '-U', DB_USER, '-d', DB_NAME, '-r', 'xml', '-H', DB_HOST, '-P', DB_PORT, XML_NAME],
                shell=True, check=True)
